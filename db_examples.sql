...
-- -------------------------------------------------------------
--  trg_check_stock_before_expense
--  BEFORE INSERT ON product_expense
--  Проверяет что суммарный остаток по партии не меньше
--  запрашиваемого количества. Откатывает транзакцию с ошибкой.
-- -------------------------------------------------------------
...
BEFORE INSERT ON product_expense
FOR EACH ROW EXECUTE FUNCTION check_stock_before_expense();


-- -------------------------------------------------------------
--  trg_reduce_stock_on_expense
--  AFTER INSERT ON product_expense
--  Уменьшает остаток на складе после успешной записи расхода.
-- -------------------------------------------------------------
...
AFTER INSERT ON product_expense
FOR EACH ROW EXECUTE FUNCTION reduce_stock_on_expense();
...
-- -------------------------------------------------------------
--  trg_update_status_on_departure
--  BEFORE UPDATE OF departure_date ON cat
--  При заполнении даты выбытия автоматически меняет статус
--  кошки на «пристроен».
-- -------------------------------------------------------------
...
BEGIN
    IF NEW.departure_date IS NOT NULL AND OLD.departure_date IS NULL THEN
        UPDATE cat
        SET status_id = (SELECT status_id FROM cat_status WHERE name = 'пристроен')
...
BEFORE UPDATE OF departure_date ON cat
FOR EACH ROW EXECUTE FUNCTION update_status_on_departure();
