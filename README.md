...
- **[PostgreSQL 17](https://www.postgresql.org/download/)** — при установке запомните пароль пользователя `postgres`
- **Браузер** — Chrome, Edge или Firefox

---

## Быстрый старт — восстановление готовой БД (рекомендуется)

В репозитории есть файл `db_seed.sql` — полный дамп базы с тестовыми данными.  
Одна команда создаёт БД с нуля и загружает все данные:

```bash
psql -U postgres -f db_seed.sql
```

> Требуется psql 17 (входит в состав PostgreSQL 17). Команда удалит существующую БД `catbd` и создаст новую.

После этого сразу переходите к [Шагу 4](#шаг-4--настроить-строку-подключения).

**Тестовые аккаунты в дампе:**

| Логин | Пароль | Роль |
|---|---|---|
| `ivanova` | `pass123` | Директор |
| `sidorova` | `pass123` | Администратор |
| `petrov` | `pass123` | Ветеринар |
| `smirnova` | `pass123` | Менеджер |
